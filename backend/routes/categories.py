...
def get_categories():
    with session_scope() as session:
        categories = session.query(Category).all()  # Consulta todas as categorias
        response = [
...
def create_category():
    ...
    with session_scope() as session:
        existing = (
            ...
            .first()  # Consulta se a categoria já existe
        )
        ...
        session.add(new_category)  # Adiciona a nova categoria à sessão
        session.flush()  # Garante que a nova categoria tenha um ID gerado
...
def update_category(category_id):
    ...
    with session_scope() as session:
        ...
        response = CategoryResponse.model_validate(category).model_dump()
        return jsonify(response), 200
...
def delete_category(category_id):
    with session_scope() as session:
        ...
        session.delete(category)  # Deleta categoria
